...
public:
  ...
  void yaw_callback(const std_msgs::Float64::ConstPtr& msg){
	...
	void location_callback(const std_msgs::Float64MultiArray::ConstPtr& msg)	{
    //cout<<"get current location"<<endl;
    x_current = msg->data[0];
    y_current = msg->data[1];
    ...
    clock_t start = clock();
		float d[2][2];
    double dd[4] = {450663.4357280457, 3951713.3662974704,450507.7103649403, 3951695.857893395};
    int cnt = 0;
/*
    for(int i=0; i<2; i++){
      if (k){continue;}
      else{
        d[cnt][0] = cos(yaw_current*PI/180)*(msg.data[i*2]-x_current)-sin(yaw_current*PI/180)*(msg.data[i*2+1]-y_current);
        d[cnt][1] = sin(yaw_current*PI/180)*(msg.data[i*2]-x_current)+cos(yaw_current*PI/180)*(msg.data[i*2+1]-y_current);
        cnt++;
      }
...
/*
    ...
    d[1][1] = 0;
*/
    cout<<"1 : "<<d[0][0]<<","<<d[0][1]<<endl;
    cout<<"2 : "<<d[1][0]<<","<<d[1][1]<<endl;
    ...
    if(d[0][0] == 0 or d[1][0] ==0){
      ...
      return;
		...
		double l_center =max(-25.0, min(25.0, A*400+B*20));
		double l_heading = atan(2*A*20+B);
		double l_width = 10.0;
		double v_width = 1.16;
		double d0 = 20;
		int nxy = 10;
    ...
    l.header.frame_id = dest.header.frame_id = "/my_frame";
