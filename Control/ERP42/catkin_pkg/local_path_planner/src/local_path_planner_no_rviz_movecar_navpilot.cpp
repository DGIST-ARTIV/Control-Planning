...
public:
  ...
  ros::Subscriber gps_yaw_sub;
	ros::Subscriber nav_speed_sub;
	double v{5}, steer{0}, desired_v{velocity};
	double x_current = 0;
  double count= 0 ;
  double y_current = 0;
  ...
  Mat local_map = Mat::zeros(500, 500, CV_8UC1);
	...
	Planning(ros::NodeHandle * nodeHandle){
		...
		reset_map(local_map);
		ROS_INFO("initializing nav pilot");
    ...
    gps_yaw_sub = nh.subscribe("/opencr/imu_yaw", 1, &Planning::yaw_callback, this);
		nav_speed_sub = nh.subscribe("/mission_manager/nav_speed", 1,&Planning::speed_callback, this);
	...
	void speed_callback(const std_msgs::Int16::ConstPtr &msg){
		if (msg->data == 0){desired_v = velocity; return;}
		desired_v = double(msg->data);
	}
  ...
  //  cout<<"get current location"<<endl;
	...
	void move_car(double x, double y, double yaw, double yaw_init, int type){
		...
		int acc = max(0,int(double(desired_v) - double(abs(steer))/max_steer*desired_v/3.0));
		std_msgs::Int16 acc_;
...
int main(int argc, char* argv[]) {
	cout<<"Run Nav Pilot"<<endl;
	ros::init(argc, argv, "nav_pilot");
	ros::NodeHandle nh;
