...
def check_map(map, x,y):
    if (250-round(y*10))<0 or (250-round(y*10))>499 or (round(x*10)<0) or (round(x*10)>499) : return False
    elif map[250 - round(y*10)][round(x*10)]==1: return True
    else: return False

#pi가 넘는 각도를 다시 pi로 옮김
def pi_2_pi(angle):
...
def generate_trajectory_costmap(s, km, kf, k0, map):
    cnt=0
    n = s / ds
    time = s / v  # [s]
    cost = 0
    ...
    for ikp in kp:
        ...
        y.append(state.y)
        yaw.append(state.yaw)
        if check_map(map, state.x, state.y):
            cnt+=1
        cost = cost + map[250 - round(state.y*10)][round(state.x*10)]
    if cnt>0:
        return x,y,yaw,True,cost
    return x, y, yaw, False, cost
