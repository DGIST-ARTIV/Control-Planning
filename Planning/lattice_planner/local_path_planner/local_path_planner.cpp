...
/*	void lidar_obstracle_callback(const tracking_msg::TrackingObjectArray& msg) {
  ...
  //  cout<<"get current location"<<endl;
    ...
    /*for(int i=0; i<10; i++){
      ...
      bool k = check_map(local_map, msg.data[i*2]-x_current, msg.data[i*2+1]-y_current);
      if (k){continue;}
      else{
        d[cnt][0] = msg.data[i*2]-x_current;
        d[cnt][1] = msg.data[i*2+1]-y_current;
        cnt++;
      }
		...
		double l_center =max(-25.0, min(25.0, A*100+B*10));
