...
public:
  ...
  Mat local_map = Mat::zeros(500, 500, CV_8UC1);
	...
	Planning(ros::NodeHandle * nodeHandle){
		...
		path_pub = nh.advertise<path_msgs::path>("/local_map/path", 1);
  ...
  //  cout<<"get current location"<<endl;
    ...
    if(result.size()==0){
      cout<<"no path generated"<<endl;
			if (count == 0){return;}
			int k = x_last.size();
			path_msgs::path path_msg;
			path_msg.x.data = x_last;
			path_msg.y.data = y_last;
			path_msg.yaw.data = yaw_last;
