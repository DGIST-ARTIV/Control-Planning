...
double Kp_beta = -3;
//PID Gain
double Kp{0.5}, Ki{0.01}, Kd{0.01}, delta_time{0.1},aw{0.0025};
			...
			double c2 = 0;
      		for(int i=0; i<x.size(); i++){
				...
				for(int j=0; j<obstacles.size(); j++){
					double d = sqrt((obstacles[j].xc+obstacles[j].v_x*dt*i - x[i])*(obstacles[j].xc+obstacles[j].v_x*dt*i - x[i])+(obstacles[j].yc+obstacles[j].v_y*dt*i - y[i])*(obstacles[j].yc+obstacles[j].v_y*dt*i - y[i]));
					//cout<<"!!!!!!!!!!!!!!!!!!!!!!! : "<<d<<","<<obstacles[j].R<<endl;
					circle(visualization_map, Point((obstacles[j].xc+obstacles[j].v_x*dt*i)*10, 250 - (obstacles[j].yc+obstacles[j].v_y*dt*i)*10), int(obstacles[j].R)*10+10, Scalar(255*i/x.size()), FILLED, LINE_8);
					if (d<obstacles[j].R){
						c2 = 11111;
						cout<<"check"<<endl;
						break;
					}
					else if((d-obstacles[j].R)<0.7){
						c2+= d-obstacles[j].R;
						cout<<"!!!"<<c2<<", "<<d<<","<<obstacles[j].R<<endl;
					}
				}
				if(c==255 || c2 == 11111){
					for(int j=0; j<x.size(); j++) circle(visualization_map, Point(x[j]*10, 250 - y[j]*10), 1, Scalar(150), FILLED, LINE_8);
				 	cout<<"check2"<<endl;
				 	*xp = 0;
				 	return 0;
				}
...
public:
	...
	ros::Subscriber curve_info;
	ros::Subscriber vellim_sub;
	...
	double prev_steer{0}, prev_error{0}, I_error{0};
	int curve_check = 0;
	...
	Planning(ros::NodeHandle * nodeHandle){
		...
		flag_pub = nh.advertise<std_msgs::Int16>("/planner/flag", 1);

		curve_info = nh.subscribe("/hdmap/mission_info",1, &Planning::mission_callback, this);
    	state_sub = nh.subscribe("/Ioniq_info", 1, &Planning::state_callback, this);
	    gps_yaw_sub = nh.subscribe("/yaw", 1, &Planning::yaw_callback, this);
    	...
    	vellim_sub = nh.subscribe("/smartcruise/desired_velocity", 1, &Planning::vellim_callback, this);	
	}
	void vellim_callback(const std_msgs::Float32::ConstPtr & msg){
		velocity = msg->data;
	}
	void mission_callback(const std_msgs::Float32MultiArray::ConstPtr & msg){
		...
		if(size == 0){
			...
			for(int i=0; i<size; i++){
				if((msg->data[3*i+1]==5 || msg->data[3*i+1] == 6 )&& (msg->data[3*i+2] >-3 && msg->data[3*i+2]<=5)){
					curve_check = 1;
					return;
				}
			}
		...
		if(obstacle_set == 0){
			...
			for (int i = 0; i < size; i++) {
				if(msg.array[i].state == 0){
					...
					obstacle.xc = (msg.array[i].bev.data[0] + msg.array[i].bev.data[4])/2;
					obstacle.yc = (msg.array[i].bev.data[1] + msg.array[i].bev.data[5])/2;
					...
					obstacle.v_x = msg.array[i].velocity.x+v/3.6;
	...
	double PID(double error){
		I_error += Ki*error*delta_time;
		I_error = max(-aw, min(aw, I_error));
		double D_error = Kd*(error - prev_error)/delta_time;
		prev_error = error;
		char str[100];
		sprintf(str, "P : %lf I : %lf D : %lf", Kp*error, I_error, D_error);
		putText(visualization_map, str, Point(0, 490), 1, 1, Scalar(255));
		return Kp*error + D_error + I_error;
	}
	int steer_filter(int steer){
		int filtering = max(3, int(1.5*max_steer * double(steer)/max_steer*double(steer)/max_steer));
		//cout<<steer<<","<<max_steer * double(steer)/max_steer*double(steer)/max_steer<<endl;
		if(abs(steer - prev_steer)<filtering || abs(steer - prev_steer)>70){		
			return steer;
		}
		else if(steer - prev_steer>filtering){
			return prev_steer + filtering;
		}
		else{
			return prev_steer - filtering;
		}
  ...
  //  cout<<"get current location"<<endl;
	...
	void move_car(double x, double y, double yaw, double yaw_init, int type){
		...
		int cmd_steer = -max(-max_steer, min(max_steer, int(cmd_w * 180/PI/Alpha)));
		//printf("%d %d %lf\n",max_steer, steer,double(abs(steer))/max_steer);
		int acc = max(5,int(double(velocity) - double(abs(steer))/max_steer*velocity*1));
		std_msgs::Int16 acc_;
		std_msgs::Int16 steer_;
		std_msgs::Int16 break_;
		acc_.data =  acc*10;
		int filter_steer = steer_filter(steer);
		steer_.data = filter_steer;

		prev_steer = filter_steer;

		char str[100], str1[100];
		sprintf(str, "acc : %d, steer : %d, alpha : %lf, beta : %lf", acc, steer,alpha,beta);
		sprintf(str1, "filter : %d cmd steer : %d calc steer : %d",filter_steer, cmd_steer,steer);
		...
		if(car_type == 0){
			if(cmd_type == 1){
				if(type == 0){
					...
					move_car_pub.publish(move);
					*/
					ROS_WARN("Waring tracking incorrect");
				}
				else if(type == 2){
					/*
					std_msgs::Float32MultiArray move;
	...
	void destination_callback(const sensor_msgs::JointState::ConstPtr& msg)	{
		...
		double k0 = -steer*Alpha*PI/180; //check this value, this value is for ERP42
